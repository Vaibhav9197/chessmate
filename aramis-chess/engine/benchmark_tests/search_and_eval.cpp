...
namespace Test::BenchMark {
    ...
    // THANKS STOCKFISH :)
    static const std::string fishTestFENs[] {
    ...
    void staticSearch(Searcher& thread, int depth) {
        ...
        for (std::string FEN: fishTestFENs) {
            thread.resetGame();
            ...
            while (std::getline(outputStream, line)) {
                ...
                if (tokens.size() > 6 && tokens[6] == "nodes") {
                    if ( std::stoi(tokens[5]) == depth ) { // only count the nodes at the very end
                        ...
                        const double branchingRatio { (std::pow(dummyNodes , 1 / static_cast<double>(depth) )) };
    ...
    void staticEval() {
        constexpr int numEntries { sizeof(fishTestFENs) / sizeof(std::string) };
